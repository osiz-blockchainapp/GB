...
# Function: to start, stop and restart lgcy.
...
JAR_FILE_PATH="./build/libs/lgcy.jar"
PID_FILE_PATH="lgcy.pid"
LOG_FILE_PATH="lgcy.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start lgcy."
        else
            echo "Failed to start lgcy."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop lgcy."
        else
            echo "Failed to stop lgcy."
        fi
    ;;
    restart)
