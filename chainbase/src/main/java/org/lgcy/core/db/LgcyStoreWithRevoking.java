...
import org.tron.core.db2.core.ILgcyChainBase;
...
@Slf4j(topic = "DB")
public abstract class LgcyStoreWithRevoking<T extends ProtoCapsule> implements ILgcyChainBase<T> {
  ...
  protected LgcyStoreWithRevoking(String dbName) {
  ...
  protected LgcyStoreWithRevoking(DB<byte[], byte[]> db) {
    int dbVersion = CommonParameter.getInstance().getStorage().getDbVersion();
    if (dbVersion == 2) {
  ...
  // only for test
  protected LgcyStoreWithRevoking(String dbName, RevokingDatabase revokingDatabase) {
    this.revokingDB = new RevokingDBWithCachingOldValue(dbName,
        (AbstractRevokingStore) revokingDatabase);
  }

  // only for test
  protected LgcyStoreWithRevoking(String dbName, Options options,
